...
select
	(case when (sum(case when order_id is null then 1 else 0 end)) > 0 then True else False end) as is_orderId_null,
	(case when (sum(case when payment_sequential is null then 1 else 0 end)) > 0 then True else False end) as is_payseq_null,
...
/*
Need to clean Payment table for orders that were removed in the Orders clean up
*/
select *
from payment
where order_id not in (select order_id from orders); -- 3,138 orders to delete

delete from payment
where order_id not in (select order_id from orders);

select count(distinct order_id) from payment; -- customer: 96,469 unique orders
------------------------------------------------------------------------------------------------------------------------
